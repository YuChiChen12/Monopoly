...
function AssetPerStop() {
  ...
  const [showTeam, setTeam] = useState(true);
  ...
  return (
    <div className='page-container'>
      {showTeam && (
        <>
          <div className='table-title'>各小現況</div>
          {data && (
            <table>
              <thead>
                <tr>
      ...
      {!showTeam && (
        <>
          <div className='table-title'>各關置產情形(小隊在各關的置產數量)</div>
          {data && (
            <table>
              <thead>
                <tr>
      ...
      <button onClick={() => setTeam(!showTeam)}>
        {showTeam ? "查看各關置產" : "查看小隊資產"}
      </button>
      {/* Display the data
      {data && (
        <div>
